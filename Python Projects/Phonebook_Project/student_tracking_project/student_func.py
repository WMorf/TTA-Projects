...
def first_run(self):
    ...
    with conn:
        ...
        if count < 1:
            cur.execute("""INSERT INTO tbl_student (col_fname,col_lname,col_fullname,col_phone,col_email,col_course) VALUES (?,?,?,?,?,?)""", ('John','Doe','John Doe','111-111-1111','jdoe@email.com','C++'))
            conn.commit()
    conn.close()
    ...
    if (len(var_fname) > 0) and (len(var_lname) > 0) and (len(var_phone) > 0) and(len(var_email) > 0): # enforce the user to provide both names
        ...
        with conn:
            ...
            if chkName == 0: # if this is 0 then there is no existance of the fullname and we can add new data
                ...
                cursor.execute("""INSERT INTO tbl_student (col_fname,col_lname,col_fullname,col_phone,col_email,col_course) VALUES (?,?,?,?,?,?)""",(var_fname,var_lname,var_fullname,var_phone,var_email))
...
def onDeleted(self):
    ...
    self.txt_phone.delete(0,END)
    self.txt_email.delete(0,END)
    self.text_curcourse.delete(0,END)
##    onRefresh(self) # update the listbox of the changes
    try:
