import numpy as np
import matplotlib.pyplot as plt #Added to help visualize
...
def ant_colony_optimization(num_iterations):
    ...
    for iteration in range(num_iterations):
        ...
        for ant in range(num_ants):
            ...
            print(f"Iteration {iteration + 1}, Ant {ant + 1}: Path: {ant_paths[ant]}") #Visualize
...
# Visualization
# def plot_paths(paths, distances, iteration, cities_coordinates):
#     plt.figure(figsize=(10, 6))
#     for path in paths:
#         plt.plot(cities_coordinates[path, 0], cities_coordinates[path, 1], '-o')

#     plt.title(f"Iteration {iteration+1} - Best distance: {min(distances):.2f}")
#     plt.xlabel("X Coordinate")
#     plt.ylabel("Y Coordinate")
#     plt.show()

# # Generate some coordinates for the cities for visualization purposes
# cities_coordinates = np.random.rand(num_cities, 2) * 10

# # Modify your ant_colony_optimization function to call plot_paths
# # ...

# num_iterations = 100
# best_path, best_distance = ant_colony_optimization(num_iterations)

# # Plotting the best path at the end
# plt.figure(figsize=(10, 6))
# plt.plot(cities_coordinates[best_path, 0], cities_coordinates[best_path, 1], '-o')
